...
if( isset($_GET['code']) && $_GET['code'] == 'Comfort') {
	...
	if( isset($_GET['carrier']) ) {
		...
		$sts_result_carriers_pick_edit['nodelete'] = true;
		$match = $rslt->get_match();
		
		echo $rslt->render( $sts_result_carriers_pick_layout, $sts_result_carriers_pick_edit, false, false );
	} else
	if( isset($_GET['driver']) ) {
		$driver_table = sts_driver::getInstance($exspeedite_db, $sts_debug);
		
		echo $driver_table->matching( $_GET['driver'] );
	} else
	if( isset($_GET['tractor']) ) {
		$tractor_table = sts_tractor::getInstance($exspeedite_db, $sts_debug);
		
		echo $tractor_table->matching( $_GET['tractor'] );
	} else
	if( isset($_GET['trailer']) ) {
		$trailer_table = sts_trailer::getInstance($exspeedite_db, $sts_debug);
		
		echo $trailer_table->matching( $_GET['trailer'] );
	}
}
