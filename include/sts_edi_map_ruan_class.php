...
class sts_edi_map_ruan extends sts_edi_map {
	private $RUAN_id = 'RUAN';
	...
	private $edi_214_stop_event = array(
		...
		'Depart Shipper'	=> 'AF',
		'Arrive Consignee'	=> 'X1',
		'Depart Consignee'	=> 'CD'	// Note Penske has D1
	);
	...
	//! Extract needed information from the heading of the RUAN 204 EDI
	public function map_ruan_204_heading( $notdef, $heading ) {
		$heading_fields = array();
		
		if( ! $notdef )
			throw new Exception('map_ruan_204_heading: missing Not Defined');
		...
		// RUAN said not to trust this, it was hard coded to LT
		...
		$contact = $this->lookup_edi_path( $heading, 'loop100/G61' );
		if( $contact ) {
		...
		if( is_array($notes) && ! empty($notes) ) {
			...
			foreach( $notes as $row ) {
				if( isset($row["NTE01"]) && $row["NTE01"] == 'INT' && ! empty($row["NTE02"]) )
					$note[] = $row["NTE02"];
				if( isset($row["NTE01"]) &&
	...
	private function map_ruan_details( $stop ) {
		$detail_lines = array();
		$loop320 = $this->lookup_edi_path( $stop, 'loop320', EXPECT_MULTIPLE );
		if( $loop320 ) {
			foreach( $loop320 as $item ) {
				
				//! IMPORTANT - talks with RUAN indicate there is a 1 to 1 mapping of
				// L5 segments to loop350/OID
				$l5s = $this->lookup_edi_path( $item, 'L5', EXPECT_MULTIPLE );
				$loop350s = $this->lookup_edi_path( $item, 'loop350', EXPECT_MULTIPLE );
				if( $this->debug ) echo "<p>".__METHOD__.": loop320 L5=".count($l5s).", loop350=".count($loop350s).".</p>";
				//if( count($l5s) <> count($loop350s) )
				//	throw new Exception(__METHOD__.': loop320, count of L5 ('.count($l5s).') <> count of loop350/OID ('.count($loop350s).') \n talks with RUAN indicate there is a 1 to 1 mapping');
				//for( $c=0; $c<count($l5s); $c++ ) {
				//	$l5 = $l5s[$c];
				...
				//	if( $l5 && isset($l5['L502']) ) {
				//		$detail_fields["DESCRIPTION"] = $l5['L502'];
				//	}
				//	if( $l5 && isset($l5['L503']) ) {
				//		$detail_fields["CODE"] = $l5['L503'];
				//	}
					...
					if( $oid && isset($oid['OID04']) ) {//!WIP Item code
						$unit = $this->get_field_value( "OID04", $oid['OID04'] );
						$detail_fields["PIECES_UNITS"] = $this->lookup_item_units( $unit );
					}
					if( $oid && isset($oid['OID05']) ) {
						$detail_fields["PIECES"] = intval($oid['OID05']);
					}
					if( $oid && isset($oid['OID07']) ) {
						$detail_fields["WEIGHT"] = intval($oid['OID07']);
					}
	...
	private function map_ruan_204_dates( $stop, $prefix, &$fields ) {
	...
	private function map_ruan_204_stop( $stop, $prefix, &$fields ) {
		...
		$contact = $this->lookup_edi_path( $stop, 'loop310/G61' );
		if( $contact ) {
			if( isset($contact['G6102']) )
		...
		$this->map_ruan_204_dates( $stop, $prefix, $fields );
		
		if( $prefix == 'SHIPPER') {
			$detail_lines = $this->map_ruan_details( $stop );
		
			//! Pallets info
			$pld = $this->lookup_edi_path( $stop, 'PLD' );
			if( $pld && isset($pld['PLD01']) ) {
				$shipment_fields["PALLETS"] = intval($pld['PLD01']);
				if( count($detail_lines) == 1) {
					$detail_lines[0]["PALLETS"] = intval($pld['PLD01']);
				}
			}
	...
	private function map_ruan_204_shipment( $detail ) {
		$shipments = array();
		$stops = array();
			...
			if( $stop_is_a_load ) {	//! Create a new shipment based on this stop
				...
				while( $this->got_details_left( $details ) &&
					...
					if( $found_match == MATCH_EXACT ) {	// assign drop stop to shipment
						...
						$this->map_ruan_204_stop( $stop, $prefix, $shipments[$shipment_number] );
						...
						//unset($new_stop["EDI_204_S501_STOP"]);
	...
	private function map_ruan_204_summary( $summary ) {
		$summary_fields = array();
		
		if( ! $summary )
			throw new Exception('map_ruan_204_summary: missing Summary');
	...
	//! Take a RUAN 204 and work out shipments, stops etc.
	public function map_204( $client, $notdef, $edi_204 ) {
		...
		$heading_fields = $this->map_ruan_204_heading( $notdef, $this->lookup_edi_path( $edi_204, 'Heading' ) );
		
		list($shipments, $stops) = $this->map_ruan_204_shipment( $this->lookup_edi_path( $edi_204, 'Detail') );

		$summary_fields = $this->map_ruan_204_summary( $this->lookup_edi_path( $edi_204, 'Summary' ) );
		...
		*/
		...
		if( count($stops) == 2 && count($shipments) == 1 ) {
			...
			$shipment["BILLING_STATUS"] = $this->shipment_table->billing_behavior_state["entry"];
	...
	*/
	...
	public function create_990( $client_id, $our_id, $terminator, $separator, $shipment_id, 
		...
		$edi_990 .= "B1*".$our_id."*".$shipment_id."**".$action."\n";
	...
	public function create_214( $client_id, $our_id, $terminator, $separator, $shipment_id, $stop_number, $stop_event, $event_status,
		$event_timestamp, $city, $state, $primary, $control = 1, $usage = 'P' ) {
		...
		$edi_214 .= "LX*".$stop_number."\n";

		if( in_array($stop_event, array('AA', 'AB') ) )
			$edi_214 .= "AT7***".$stop_event."*".$event_status;
		else
			$edi_214 .= "AT7*".$stop_event."*".$event_status."**";
		$edi_214 .= "*".date("Ymd", $event_timestamp)."*".date("Hi", $event_timestamp)."\n";

		$edi_214 .= "MS1*".$city."*".$state."\n";
		
		// Don't bother with MS2
		
		$edi_214 .= "SE*6*".sprintf("%04d", $control)."\n";
	...
	//! Prepare a file name per RUAN document
