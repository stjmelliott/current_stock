...
// $Id: exp_addmessage.php 3884 2020-01-30 00:21:42Z duncan $
// Add message
...
if( ! $message_table->multi_company() ) {
	...
	$sts_form_addmessage_form['layout'] = preg_replace('/'.$match.'/s', '',
		$sts_form_addmessage_form['layout'], 1);	
}


$message_form = new sts_form( $sts_form_addmessage_form, $sts_form_add_message_fields, $message_table, $sts_debug);

if( isset($_POST) && count($_POST) > 0 ) {		// Process completed form
	$result = $message_form->process_add_form();

	if( $sts_debug ) die; // So we can see the results
	if( $result ) 
		if( isset($_POST["saveadd"]) )
			reload_page ( "exp_addmessage.php" );
		else
