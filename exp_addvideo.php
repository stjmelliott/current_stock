...
$video_form = new sts_form( $sts_form_addvideo_form, $sts_form_add_video_fields, $video_table, $sts_debug);

if( isset($_POST) && count($_POST) > 0 ) {		// Process completed form
	$result = $video_form->process_add_form();

	if( $sts_debug ) die; // So we can see the results
	if( $result ) 
		if( isset($_POST["saveadd"]) )
			reload_page ( "exp_addvideo.php" );
		else
